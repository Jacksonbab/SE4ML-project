...
class MultiDirectoryDataSequence(data.Dataset):
    ...
    def __init__(self, root, image_size=(336,188), transform=TRANSFORM, robustification=False, noise_level=10, num_duplicates=3, duplicate_threshold=0.2):
    ...
    def get_image_by_steering_angle(self, steering_angle):
        ...
        for i in range(len(self)):
            if abs(self.get_steering_angle(self.csv_data[i]) - steering_angle) < 0.03:
                return self.all_image_paths[i]
        return None
